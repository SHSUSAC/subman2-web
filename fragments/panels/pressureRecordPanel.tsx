...
export default function PressureRecordPanel({
	...
	panelOpen: boolean;
	savePressureRecord: (record: { record: PressureRecord; parentId: string }) => void;
	pressureRecordForm: UseFormReturn<{ record: PressureRecord; parentId: string }, object>;
}) {
	return (
		<PanelDialog open={panelOpen} title="Changing cylinder pressure">
			<FormProvider {...pressureRecordForm}>
				<form
					onSubmit={pressureRecordForm.handleSubmit((data) =>
						savePressureRecord(data as { record: PressureRecord; parentId: string })
					)}
