...
import { BCD } from "../../../lib/types/equipment";
...
const useCollection = (suspense = true) =>
	useFirestoreCollectionData(query(collection(useFirestore(), "bcds")), {
		idField: "id",
		suspense: suspense,
	});
...
export default function Index(): JSX.Element | null {
	...
	const [deleteItem, setDeleteItem] = useState<BCD | null>(null);
	const [editItem, setEditItem] = useState<BCD | null>(null);
	const log = useLog();

	const [appError, setAppError] = useState<AppError>();

	const editEquipmentForm = useForm<BCD>();

	const columns = useMemo<Column<BCD>[]>(
		() => [
			{
				Header: "Tag",
				accessor: "userTag",
			...
			{
				Header: "Actions",
				Cell: function ActionCell(props: Cell<BCD>) {
					return (
						<div className="flex items-center p-4 space-x-2">
							...
							<Link href={`/equipment/bcd/records?id=${encodeURIComponent(props.row.original.id)}`}>
								<a
	...
	const saveNewEquipment = async (newItem: BCD) => {
		log.info("Adding new bcd, values: %j", newItem);
		try {
			await toast.promise(addDoc(collection(firestore, "bcds"), newItem), {
				pending: "Saving item...",
			...
			err.code = 600;
			log.error(err, "Error adding new bcd to firestore collection");
			setAppError(err);
		} finally {
	...
	const writeEquipmentEdit = async (d: BCD) => {
		try {
			await toast.promise(updateDoc(doc(firestore, "bcds", editItem?.id ?? ""), d), {
				pending: "Saving item...",
				success: "Saved successfully",
				error: "Error saving",
			});
	...
	return (
		<>
				...
				<div className="w-full flex flex-col px-4 py-6 space-y-6 bg-white rounded-md dark:bg-darker flex-grow overflow-y-auto overscroll-y-contain">
					...
					<div className="block md:flex mb-4 gap-8">
						...
						<button
							...
							onClick={async () => {
								try {
									await toast.promise(deleteDoc(doc(firestore, "bcds", deleteItem?.id ?? "")), {
			...
			<PanelDialog open={editItem !== null} title="Editing bcd">
			...
			<main>
				<header className="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
					...
					<div>
						...
						>
							<>
								<span className="sr-only">Add bcd</span>
			...
			<LogProvider name="NewDialog">
				{
					...
					<NewEquipmentModal
						open={newPanelOpen}
						save={async (c) => await saveNewEquipment(c as BCD)}
